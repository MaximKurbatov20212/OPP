...
void mul(double* R, Matrix& A, double* X, int N, int number_of_lines, int rank, int size) {
    ...
    int start = get_number_of_first_line_in_matrix(N, rank, size); // номер, с какого надо начинать читать строку в матрице
...
void f(Matrix A, double* X_n, double* piece_of_vector_2, int number_of_lines, double* B, int N, int size, int rank) {
    ...
    for(int i = 0; i < size; i++) {                                             // цикл по процессам 
        ...
        current_rank = ((current_rank - 1) + size) % size;                               // ранг процесса с которого кусочек вектора X сейчас
    }

    sub(D, D, B, number_of_lines);    // Ax - B
